...
// Don't redeclare API_KEY if it already exists to avoid conflicts with other scripts
if (typeof window !== 'undefined' && !window.ENHANCED_MAP_API_KEY) {
    window.ENHANCED_MAP_API_KEY = "fc-4bd96b21a1fa459a9336127ab8974234";
}
const ENHANCED_MAP_API_KEY = typeof window !== 'undefined' ? 
    window.ENHANCED_MAP_API_KEY : "fc-4bd96b21a1fa459a9336127ab8974234";
...
async function mapWebsiteAsync(url, options = {}, progressCallback = () => {}) {
    ...
    try {
        ...
        diagnostics.urlsBySource.crawl = 3;
        diagnostics.urlsBySource.map = 1;
        diagnostics.urlsBySource.sitemap = 1;
        
        // Simulate fetch from /map endpoint
        progressCallback({
            phase: 'mapping',
            message: 'Fetching URLs from /map endpoint...',
            progress: 50,
            diagnostics
        });
        ...
        progressCallback({
            phase: 'sitemap',
            message: 'Checking for sitemap.xml...',
            progress: 60,
            diagnostics
        });
        ...
        // Add some sample URLs based on options
        if (options.jsRendering) {
            allUrls.push(url + 'dynamic-content');
        }
        
        if (options.allowBackwardLinks) {
            allUrls.push(url.replace(/\/$/, '') + '/../parent-directory');
        }
        
        // Simulate depth-based crawling
        if (options.maxDepth && options.maxDepth > 1) {
            for (let i = 1; i <= Math.min(options.maxDepth, 3); i++) {
                allUrls.push(url + 'level' + i + '/page');
            }
        }
        
        // Limit URLs if required
        if (options.limit && allUrls.length > options.limit) {
            allUrls = allUrls.slice(0, options.limit);
            diagnostics.warnings.push({
                type: 'url_limit_reached',
                message: `Hit the URL limit of ${options.limit}. Some URLs may be missing.`
            });
        }
        
        // Update diagnostics
        diagnostics.urlsDiscovered = allUrls.length;
        
        // Simulate ranking
        progressCallback({
            phase: 'ranking',
            message: 'Ranking URLs by importance...',
            progress: 90,
            diagnostics
        });
        
        // Actually rank the URLs
        ...
        progressCallback({
            phase: 'complete',
            message: `Mapping complete! Found ${rankedUrls.length} URLs.`,
            progress: 100,
            urlCount: rankedUrls.length,
            diagnostics
        });
