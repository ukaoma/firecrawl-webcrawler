 ...
 * 
 * Note: This file uses .mjs extension for ES modules support
 */
...
function extractDataMultipleWays(response) {
  ...
  // Approach 5: Look for success: true, data: object format (common API pattern)
  if (response && response.success === true && response.data) {
    console.log('Approach 5 (success + data pattern): Found success=true with data object');
    
    if (response.data.zip_data && Array.isArray(response.data.zip_data)) {
      console.log(`  Found zip_data array in data with ${response.data.zip_data.length} entries`);
      extractedData = extractedData.concat(response.data.zip_data);
    }
  }
  ...
  if (extractedData.length > 0) {
    ...
    console.log('Inspect the RAW API RESPONSE to understand the structure.');
  }
}
